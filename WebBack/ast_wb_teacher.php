                    ...
                    <div class="modal-header">
                      ...
                      <div class="modal-body">
                        ...
                        <div class="form-group">
                          ...
                          <input type="text" class="form-control" id="account_name" value="" placeholder="請輸入手機號碼" required>
                        ...
                        <div class="form-group">
                          ...
                          <input type="text" class="form-control" id="teacher_name" value="" required>
                          <div class="invalid-feedback">
                            請輸入姓名
                          </div>
                        </div>
                        <div class="form-group">
                          ...
                          <select class="form-control" id="teacher_gender">
                        ...
                        <div class="form-group">
                          ...
                          <input type="email" class="form-control" id="account_email" value="" required>
                          <div class="invalid-feedback">
                            請輸入E-mail
                          </div>
                        </div>
                        <div class="form-group">
                          <label for="teacher_class">班級代號:</label>
                          <input type="text" class="form-control" id="teacher_class" value="" required>
                          <div class="invalid-feedback">
                            請輸入班級代號
                          </div>
                        </div>
                      ...
                      <div class="modal-footer">
                        <button type="submit" class="btn btn-danger" id="btnCreate" data-dismiss="modal">送出</button>
                      </div>
                  </div>
                </div>
              </div>
    ...
    <script>
      ...
      function fnCreate(data){
        ...
        if(data==0){      
          $.ajax({
            ...
            data: {name: $("#account_name").val(),password: $("#password").val(),identity_id: 4,email: $("#account_email").val()},
          ...
          $.ajax({
            type: "POST",
            url: "../php/api.php/c/teacher/",
            data: {account_name: $("#account_name").val(),name: $("#teacher_name").val(),gender: $("#teacher_gender").val(),class_id: $("#teacher_class").val()},
            success: function(data){
              if(data.statusCode==10200){
                // alert("create teacher success");
                history.go(0);  //重整頁面
              }else{
            ...
            error:function(){
              alert("create teacher api failed");
            }
          });
        }else{
            alert("帳號已被註冊,請重新填寫!");
      ...
      function showList(result){
        ...
        for(i=0; i<data.length; i++){
          ...
          <tr>\
            ...
            <td>\
              <a href="ast_wb_teacher_update.php?id='+data[i].id+'" class="btn btn-sm btn-primary mr5">更新</a>\
              <a href="#" class="btn btn-sm btn-danger" onclick="fnDelete('+data[i].id+')">刪除</a>\
            </td>\
          </tr>';
          //console.log(data[i]);
          //console.log("html:"+strHTML);
          $("#teacher_list").append(strHTML);
        }   
        $('#example').DataTable();   
      }
      function goUpdate(){
      ...
      function fnDelete(id){
        if(confirm("確認刪除?")){
          $.ajax({
            type: "POST",
            url: "../php/api.php/d/teacher/id/"+id,
            dataType: "json",
            success: function(data){
              if(data.statusCode==10200){
